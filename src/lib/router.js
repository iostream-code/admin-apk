// Router ringan pengganti Framework7 router -- disalin apa adanya (bentuk &
// perilaku) dari inventory-apk/src/lib/router.js, yang sendirinya meniru
// sebagian kecil API Framework7 (`navigate`, `currentRoute.url`) supaya kode
// lama yang dulu memanggil `app.views.main.router.navigate(...)` bisa
// dipindah ke `Router.navigate(...)` dengan perubahan minimal saat halaman
// lain (point sales/produksi/admin/sj, absensi, data gaji, data transaksi)
// dimigrasikan menyusul.
//
// Hash-based (window.location.hash), BUKAN history.pushState dgn path polos
// -- app ini jalan di dalam Cordova WebView (file://), dan
// pushState({path},'','/dashboard') akan mengubah URL yg terlihat jadi
// file:///dashboard, yg gagal di-reload/back-gesture (layar putih).
