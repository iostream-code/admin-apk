...
// inventory-apk/finance-apk/admin-finance-apk (lihat bump-version.cjs di sana).
...
function updateVersion(configXml, newVersion, newAndroidCode) {
  ...
  if (updated.includes("ios-CFBundleVersion=")) {
    updated = updated.replace(
      /ios-CFBundleVersion="[^"]+"/,
      `ios-CFBundleVersion="${versionString}"`,
    );
  } else {
    updated = updated.replace(
      /<widget([^>]*?)>/,
      `<widget$1 ios-CFBundleVersion="${versionString}">`,
    );
  }
...
// `>=` ke config_value_minimal (tabel `config`, config_id='VERSION_ADMIN_PUSAT').
// Integer dipilih (bukan version string) krn gampang dibandingkan, konsisten
// dgn sibling apps lain di workspace ini (finance-apk, admin-finance-apk,
// ekspedisi-apk, inventory-apk).
...
// config_value_minimal (tabel config, config_id='VERSION_ADMIN_PUSAT').
...
function bumpVersion(type = "patch", customVersion = null) {
  ...
  console.log(
    `Versi saat ini: ${currentVersion.major}.${currentVersion.minor}.${currentVersion.patch}`,
  );
  ...
  console.log(
    `PENTING: nilai di DB (tabel config, config_id='VERSION_ADMIN_PUSAT',`,
  );
  console.log(
    `  config_value_minimal) HARUS ikut di-update manual ke ${newAndroidCode} kalau rilis`,
  );
  console.log(
    `  ini memang wajib dipakai (bukan cuma opsional/rekomendasi) -- script ini`,
  );
  console.log(
    `  SENGAJA tidak push otomatis ke server (config table dipakai banyak app lain).`,
  );
}
